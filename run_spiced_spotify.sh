...
printf "CREATING THE TEMPORARY SPOTX EXECUTION FILE\n"
set -x
touch "$EXEC_LOC"
set +x
printf "\n"

printf "DOWNLOADING SPOTX CONTENT\n"
set -x
curl -ssL "$DWNLD_LOC" -o "$EXEC_LOC" # Downloading content
chmod +x "$EXEC_LOC" # Giving permissions
set +x
printf "\n"

printf "RUNNING SPOTX CONTENT\n"
set -x
printf "y\nn\ny\ny\ny\nn\n" | bash "$EXEC_LOC"
set +x
printf "\n"

if [[ -d "$SPICETIFY_LOC"]]; then
